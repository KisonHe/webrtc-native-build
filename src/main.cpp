...
rtc::scoped_refptr<webrtc::PeerConnectionInterface> get_default_peer_connection(
  ...
  auto peer_connection = std::move(error_or_peer_connection.value());
 ...
 public:
  ...
  void on_frame(I420Creator::I420Frame frame) {
    ...
    rtc::scoped_refptr<webrtc::I420Buffer> buffer = webrtc::I420Buffer::Copy(
        1920, 1080, frame->data(), 1920, frame->data() + 1920 * 1080,
        (1080 + 1) / 2, frame->data() + 1920 * 1080 + 1920 * 1080 / 4,
        (1080 + 1) / 2);
...
class DummySetSessionDescriptionObserver
 ...
 public:
  ...
  virtual void OnFailure(webrtc::RTCError error) override { assert(false); }
};
...
int main() {
  ...
  auto sender = new rtc::RefCountedObject<SimpleClient>(true);
  auto receiver = new rtc::RefCountedObject<SimpleClient>(false);
